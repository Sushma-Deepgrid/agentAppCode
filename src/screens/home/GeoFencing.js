...
const [centroid, setCentroid] = useState([]);
    ...
    useEffect(() => {
      ...
      if (response.data.task.geo_fencing && response.data.task.geo_fencing.length >= 3) {
...
setArrayposition(objDel)
  ...
  <Text style={{fontSize:18,fontWeight:'bold'}}> Plot Area: {polygonArea} sq. yd</Text>
        }
</View>
  ...
  {Arrayposition.map((marker, index) => (
          ...
          <View style={{ backgroundColor: 'none', padding: 5, borderRadius: 5 }}>
            <Text style={{ fontSize: 12, fontWeight: 'bold', color: 'black' }}>{polygonArea} sq. yd</Text>
          </View>
        </Marker>
                </>
              }
